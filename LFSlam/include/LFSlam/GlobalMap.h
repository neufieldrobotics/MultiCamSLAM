...
public:
    Landmark();
    Landmark(Mat p, LightFieldFrame* lf_frame, int featInd, Point2f uv, int id);
    Landmark(Mat p, MonoFrame* frame, int featInd, Point2f uv, int id);

    ~Landmark();

    void addLfFrame(LightFieldFrame* lf_frame, int featInd, Point2f uv);
    void addMonoFrame(MonoFrame* lf_frame, int featInd, Point2f uv);

    int lId;
    Mat pt3D;
    vector<LightFieldFrame*> KFs;
...
public:
    ...
    int insertLandmark(Mat p, LightFieldFrame* lf_frame, int featInd, Point2f uv);
