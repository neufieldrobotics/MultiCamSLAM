...
struct DetectionResult
{
  ...
  /// Score of the matched id obtained
  double score;
...
public:
  ...
  bool findTopN(const std::vector<cv::KeyPoint> &keys,
                    const std::vector<TDescriptor> &descriptors, string filename,
  ...
  inline void clear();
    ...
    inline vector<string> getFileNames()
    {
        return m_filenames;
    }
...
template<class TDescriptor, class F>
bool TemplatedLoopDetector<TDescriptor, F>::findTopN(
            const std::vector<cv::KeyPoint> &keys,
            const std::vector<TDescriptor> &descriptors, string filename,
            DetectionResult &match)
    {
        ...
        cout<<"Length of the bag of words vector: "<<bowvec.size()<<endl;
        ...
        else
        {
            ...
            QueryResults qret;
            m_database->query(bowvec, qret,5, max_id);
            ...
            if(!qret.empty())
            {
                ...
                if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)
                {
                    ...
                    for(auto& res: qret){
                        cout<<"matched image : "<<m_filenames.at(res.Id)<<" Score: "<<(res.Score/ns_factor)<<endl;
                    }
                    if(!qret.empty())
                    {
          ...
          if(!islands.empty())
          {
            ...
            if(getConsistentEntries() > m_params.k)
            {
              ...
              if(detection)
              {
                ...
                match.score = match.score/ns_factor;
              }
              else
              {
