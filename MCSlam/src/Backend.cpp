...
void Backend::optimizePosesLandmarks() {
    ...
    if(optimizationMethod == 0){
        ...
        try {
            ...
            // VLOG(3) << "Frame " << frame_id << ": " << endl;
            //    currentEstimate.print("Current estimate: ");
            // graph.print("current graph");
            VLOG(3) << "BackEnd Optimization done" << endl;
        }
...
//                    }
        ...
        if(windowCounter >= 2){
            try{
                ...
                initialEstimate.clear();
                newTimeStamps.clear();
                VLOG(3)<<"Number of factors in graph : "<<fixedLagOptimizer.getFactors().size();
                // Each call to iSAM2 update(*) performs one iteration of the iterative nonlinear solver.
                // If accuracy is desired at the expense of time, update(*) can be called additional times
                // to perform multiple optimizer iterations every step.
//            isam.update();
