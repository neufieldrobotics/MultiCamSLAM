...
//#    endif
//#    ifndef CV_BGRA2RGBA
//#        define CV_BGRA2RGBA COLOR_BGRA2RGBA
//#    endif
//#  endif
#endif // WITHOUT_NUMPY
...
// a non-numpy alternative for `detail::get_2darray()`.
    ...
    bool hist(const std::vector<Numeric>& y, long bins=10,std::string color="b",
              double alpha=1.0, bool density=false, bool cumulative=false)
    {
        detail::_interpreter::get();
        ...
        PyDict_SetItemString(kwargs, "density", density ? Py_True : Py_False);
    ...
    template< typename Numeric>
    bool hist_bins(const std::vector<Numeric>& y, std::vector<Numeric>& bins,std::string color="b",
              double alpha=1.0, bool density=false, bool cumulative=false)
    {
        detail::_interpreter::get();

        PyObject* yarray = detail::get_array(y);
        PyObject* binarray = detail::get_array(bins);
        PyObject* kwargs = PyDict_New();
        PyDict_SetItemString(kwargs, "bins", binarray);
        PyDict_SetItemString(kwargs, "color", PyString_FromString(color.c_str()));
        PyDict_SetItemString(kwargs, "alpha", PyFloat_FromDouble(alpha));
        PyDict_SetItemString(kwargs, "cumulative", cumulative ? Py_True : Py_False);
        PyDict_SetItemString(kwargs, "density", density ? Py_True : Py_False);
    ...
    void imshow(const cv::Mat &image, const std::map<std::string, std::string> &keywords = {})
{
    ...
    case 3:
        ...
        break;
    case 4:
        cv::cvtColor(image2, image2, COLOR_BGRA2RGBA);
    }
               ...
               std::map<std::string, std::string>())
    {
        ...
        PyDict_SetItemString(kwargs, "marker", pystring);
        PyDict_SetItemString(kwargs, "alpha", PyFloat_FromDouble(0.1));
        ...
        if (!plot3) throw std::runtime_error("No 3D scatter plot");
        Py_INCREF(plot3);
        PyObject *res = PyObject_Call(plot3, args, kwargs);
